"""
V2 eval — RAG on, position filter off.
ChromaDB retrieval is active but the where={"position": ...} clause is stripped
from both get_player_combine and get_position_peers, so results can cross positions.
"""
import json
import os
import sys
...
# Patch retriever before ai/* imports so the patched versions are used everywhere
import rag.retriever as _retriever

def _get_player_combine_no_filter(name: str, position: str = None) -> list[dict]:
    """Same as get_player_combine but never applies the position where-clause."""
    vector = _retriever._embed(name)
    results = _retriever._collection().query(
...
def _get_position_peers_no_filter(position: str, n: int = 5) -> list[dict]:
    """Same as get_position_peers but without the position where-clause."""
    vector = _retriever._embed(f"combine performance athlete {position}")
    results = _retriever._collection().query(
        query_embeddings=[vector],
        n_results=n,
    )
...
# Patch the retriever module in-place so ai/* pick up the unfiltered versions
_retriever.get_player_combine = _get_player_combine_no_filter
_retriever.get_position_peers = _get_position_peers_no_filter

# Now import the real pipelines — they will call the patched functions
...
def run_eval():
    ...
    out_path = Path(__file__).parent / "results" / f"run_v2_{timestamp}.json"
